...
```bash
uv tool install saferead     # recommended (isolated, upgradable)
# or
pipx install saferead
# or
pip install saferead
```
...
saferead install --project  # write to ./.claude/settings.json
saferead install ~/.claude/settings.json   # write to an explicit path
```

This merges a single `PreToolUse` entry (matcher `Bash`) into the target
`settings.json`, backing up the file first and leaving any existing hook
entries untouched. Re-running is idempotent.

You can also wire it up manually — the hook reads a PreToolUse JSON payload on
stdin and writes a decision envelope on stdout:

```json
{
  "hooks": {
    "PreToolUse": [
      { "matcher": "Bash", "hooks": [ { "type": "command", "command": "saferead" } ] }
    ]
  }
}
```
...
```bash
saferead update          # runs `uv tool upgrade saferead`
# or
