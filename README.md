...
[uv](https://docs.astral.sh/uv/) is the only prerequisite. `uvx` fetches and
runs `saferead` on demand (cached after first use) and provides the Python
3.11+ runtime — there is nothing to install.

## Enable the hook

Add `saferead` as a `PreToolUse` hook in your Claude Code settings —
`~/.claude/settings.json` (all projects) or a project's `.claude/settings.json`:
...
That's it. On each `Bash` command `uvx` runs `saferead`, which reads the
PreToolUse payload on stdin and returns a decision. To pin a version, use
`uvx saferead@0.1.1`.
...
Or pin the version in your hook command (`uvx saferead@<version>`) and bump it
