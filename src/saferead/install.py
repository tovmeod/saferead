...
def _select_target(args: list[str]) -> Path:
    ...
    ``Path(arg).expanduser().resolve()``; neither -> the global default
    ``~/.claude/settings.json``. The no-argument case installs globally
    *non-interactively* so ``saferead install`` works exactly as documented —
    no prompt to answer.
    """
    if "--project" in args:
